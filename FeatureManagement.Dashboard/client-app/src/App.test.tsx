...
const mockConfigureExperiment = vi.fn();
const mockAssignExperimentVariant = vi.fn();
const mockRecordExperimentOutcome = vi.fn();
const mockGetExperimentRecommendation = vi.fn();
...
vi.mock('./hooks/useFeatureFlags', () => ({
    useFeatureFlags: (showNotification: (message: string, severity: 'success' | 'error' | 'info' | 'warning') => void) => {
        ...
        return {
            ...
            configureExperiment: mockConfigureExperiment,
            assignExperimentVariant: mockAssignExperimentVariant,
            recordExperimentOutcome: mockRecordExperimentOutcome,
            getExperimentRecommendation: mockGetExperimentRecommendation
        };
    }
}));

vi.mock('./components/FeatureGrid', () => ({
    default: ({onEdit, onToggle, onHistory, onDelete, onActivity}: {
        onEdit: (flag: FeatureFlag) => void;
        onToggle: (flag: FeatureFlag) => void;
        onHistory: (flag: FeatureFlag) => void;
        onDelete: (flag: FeatureFlag) => void;
        onActivity: (flag: FeatureFlag) => void
    }) => (
        <div>
...
vi.mock('./components/ExperimentDialog', () => ({
    default: ({open, featureName, onClose}: { open: boolean; featureName: string; onClose: () => void }) =>
        open ? (
            <div>
                <div>experiment-open-{featureName || 'empty'}</div>
                <button onClick={onClose}>experiment-close</button>
            </div>
        ) : null
}));
...
describe('App', () => {
    beforeEach(() => {
        ...
        mockConfigureExperiment.mockResolvedValue(true);
        mockAssignExperimentVariant.mockResolvedValue({variant: 'A', bucket: 1});
        mockRecordExperimentOutcome.mockResolvedValue(true);
        mockGetExperimentRecommendation.mockResolvedValue(null);
